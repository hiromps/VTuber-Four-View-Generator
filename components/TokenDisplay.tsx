...
export default function TokenDisplay({ tokens, onBuyTokens }: TokenDisplayProps) {
  const { t } = useLanguage()

  return (
    <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
      <div className="bg-gray-700 rounded-lg px-2 py-1.5 sm:px-3 sm:py-2 flex items-center gap-1 sm:gap-1.5">
        ...
        <span className="font-bold text-white text-sm sm:text-base">{tokens}</span>
        <span className="text-gray-300 text-xs sm:text-sm hidden sm:inline">{t('tokens.tokensLabel')}</span>
      </div>
      <button
        onClick={onBuyTokens}
